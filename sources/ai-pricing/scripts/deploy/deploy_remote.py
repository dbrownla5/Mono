...
def main():
    # SSH_PASSWORD read from environment (avoid process list leak via argv).
    # deploy.bat loads .env before invoking this script.
    password = os.environ.get("SSH_PASSWORD")
    if not password:
        # Backward-compat fallback: accept argv[1] but emit a warning so
        # operators migrate. Will be removed in v0.2.
        if len(sys.argv) >= 2:
            print("[WARN] SSH_PASSWORD via argv is deprecated, use .env instead.")
            password = sys.argv[1]
        else:
            sys.exit("[ERREUR] SSH_PASSWORD env var requise (voir .env.example).")
